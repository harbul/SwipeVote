...
## Five concrete places I had to push back
...
**5. "Most Loved" containing 0% items — the difference between technically correct and semantically correct.** Claude's first Results implementation sorted each mode but didn't *filter* — so the "Most Loved" tab contained all 100 breeds, ordered by yes-rate descending. Scrolling past the top, you'd find 0% items at the bottom because they were still in the list, just last. Claude's code was technically correct (the sort was right) but semantically wrong (a tab labeled "Most Loved" shouldn't contain anything 0% loved). I screenshotted the bottom of the list and pointed it out. The fix was reframing each sort as a filtered category — "Most Loved" requires ≥ 4 votes AND ≥ 60% yes-rate; "Most Divisive" requires ≥ 5 votes AND within 20pts of 50/50; "Most Skipped" is ≤ 2 total. That kind of misalignment between *what the UI promises* and *what the implementation does* is exactly the class of bug AI can't catch on its own — it requires a human checking that the words on the screen still mean what they say.

A second bug surfaced in the same fix: when I switched sort modes, the hero card was getting stuck on the previous sort's #1. Claude had wrapped it in `<AnimatePresence mode="wait">`, but the old element wasn't exiting cleanly — even after 1.5s of waiting. Removing AnimatePresence entirely (and relying on React's key-based remount) fixed it. Worth noting because Claude initially defended the pattern; I had to insist we just remove it and try.
